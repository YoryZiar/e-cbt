...
export default function JurnalTableItem({
    ...
    return (
        <TableRow key={jurnal.id} className="border-white/10 hover:bg-white/5 transition-colors">
            ...
            <TableCell>
                <div className="font-medium w-3/6 mx-auto flex flex-col gap-2">
                    <Link href={`/user/jurnal/${jurnal.id}`}>
                        <Button className="w-full bg-secondary text-primary hover:bg-[#c4bdff]">Detail</Button>
                    </Link>
                    <Button onClick={removeJurnal} className="w-full bg-red-500/20 text-red-400 hover:bg-red-500/30">Hapus</Button>
                </div>
            </TableCell>
        </TableRow>
    )
