const fs = require('fs');
const path = require('path');

const dirs = ['cemas', 'depresi', 'emosi', 'kesepian', 'kurang-percaya-diri', 'malas', 'sedih', 'stress', 'sulit-tidur', 'takut', 'trauma'];

for (const dir of dirs) {
    const filePath = path.join(__dirname, 'app', '(home)', 'therapy', dir, 'page.tsx');
    if (!fs.existsSync(filePath)) continue;

    const content = fs.readFileSync(filePath, 'utf-8');

    // Extract title
    const titleMatch = content.match(/<h1[^>]*>(.*?)<\/h1>/i);
    const title = titleMatch ? titleMatch[1] : dir;

    // Extract intro paragraph
    const introMatch = content.match(/<p[^>]*>(.*?)<\/p>/is);
    const intro = introMatch ? introMatch[1].trim() : '';

    // Extract list items
    const lisMatch = content.match(/<ol[^>]*>(.*?)<\/ol>/is) || content.match(/<ul[^>]*>(.*?)<\/ul>/is);
    const lis = lisMatch ? lisMatch[1] : '';

    const items = [];
    const liRegex = /<li[^>]*>(.*?)<\/li>/gis;
    let match;
    while ((match = liRegex.exec(lis)) !== null) {
        items.push(match[1].trim());
    }

    const componentName = title.replace(/[^a-zA-Z0-9]/g, '');

    const newContent = `import Link from "next/link"
import { ArrowLeft, Sparkles } from "lucide-react"

function ${componentName}Content() {
    return (
        <div className="min-h-screen py-24 relative overflow-hidden flex items-center justify-center">
            {/* Background effects */}
            <div className="absolute top-0 right-1/4 w-72 h-72 bg-secondary/20 rounded-full mix-blend-screen filter blur-[100px] opacity-70 pointer-events-none"></div>

            <div className="container mx-auto px-4 relative z-10 max-w-4xl">
                <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[2.5rem] shadow-2xl p-8 lg:p-14">
                    
                    <div className="text-center mb-10">
                        <div className="w-16 h-16 bg-secondary/20 rounded-2xl flex items-center justify-center mx-auto mb-6">
                            <Sparkles className="w-8 h-8 text-secondary" />
                        </div>
                        <h1 className="text-3xl lg:text-5xl font-bold text-white mb-6">${title}</h1>
                        ${intro ? `<p className="text-slate-300 font-light leading-relaxed text-lg max-w-3xl mx-auto">${intro}</p>` : ''}
                    </div>

                    <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
                        <ul className="space-y-6">
                            ${items.map((item, index) => {
                                // Split title and description if there's a strong/bold or simple newline
                                // Usually it's just raw text, we can just render it.
                                return `<li className="flex items-start">
                                    <div className="min-w-10 h-10 rounded-full bg-secondary text-primary flex items-center justify-center font-bold mr-5 shrink-0">
                                        ${index + 1}
                                    </div>
                                    <div className="text-slate-300 font-light text-lg pt-1">
                                        ${item}
                                    </div>
                                </li>`;
                            }).join('\n                            ')}
                        </ul>
                    </div>

                    <div className="mt-12 text-center">
                        <Link href="/therapy" className="inline-flex items-center justify-center px-10 py-4 font-bold text-slate-300 bg-white/5 border border-white/10 rounded-full overflow-hidden transition-all duration-300 hover:bg-white/10 hover:text-white hover:-translate-y-1 group">
                            <ArrowLeft className="mr-2 w-5 h-5 group-hover:-translate-x-1 transition-transform" />
                            Kembali ke Menu Terapi
                        </Link>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default function ${componentName}Page() {
    return (
        <${componentName}Content />
    )
}
`;

    fs.writeFileSync(filePath, newContent, 'utf-8');
    console.log('Updated ' + dir);
}
